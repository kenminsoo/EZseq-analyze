...
#With eight cores, this pipeline runs at a speed of 45 seconds per sample.
...
fastqc unaligned/*.fastq.gz -t 8 -o output/fastqc_analysis
...
user_adapter1="AACTGTAGGCACCATCAAT"
user_adapter2=""
...
for f in *.fastq.gz ; do
    file="$f"
    sampname="${file%%.*}"
    AdapterRemoval --file1 "${file}" --adapter1 AACTGTAGGCACCATCAAT --basename "trimmed"_"${sampname}" --gzip --threads 8 --trimqualities --minlength 20
done
...
fastqc output/adaptertrim/*.fastq.gz -t 8 -o output/fastqc_post_trim
...
for f in output/adaptertrim/*.fastq.gz; do
    file="$f"
    sampname="${file%%.*}"
    bowtie2 -x unaligned/grch38_1kgmaj -U $f -S "${sampname}".sam -p 8
done
...
featureCounts -T 8 -a annotation/hsa_all.gff3 -F 'GTF' -g 'gene_name' -o "hsa_all.tsv" output/aligned/*.sam
featureCounts -T 8 -a annotation/hsa_all_smrna.gff -F 'GTF' -t 'lnc_RNA' -g 'gene_id'  -o "hsa_all_smrna.tsv" output/aligned/*.sam
featureCounts -T 8 -a annotation/hsa_mirna.gff3 -F 'GTF' -t 'miRNA' -g 'Name' -o "hsa_mirna.tsv" output/aligned/*.sam
featureCounts -T 8 -a annotation/hsa_pirna.gtf -F 'GTF' -t 'piRNA' -g 'piRNA_code' -o "hsa_pirna.tsv" output/aligned/*.sam
